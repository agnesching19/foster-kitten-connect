...
export function DashboardPage() {
  ...
  const earliestArrival = litters.length
    ? litters.reduce((min, litter) => (litter.arrived < min ? litter.arrived : min), litters[0]!.arrived)
    : null
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <section className="grid gap-4 lg:grid-cols-2" aria-live="polite">
          {visibleLitters.map((litter) => <BatchCard key={litter.id} litter={litter} />)}
        </section>
      ) : (
        <Card className="py-12 text-center">
          <p className="font-medium text-ink">{litters.length ? 'No matching batches' : 'No litters yet'}</p>
          <p className="mt-1 text-sm text-muted">
            {litters.length ? 'Try another name or filter.' : 'Add your first foster litter to get started.'}
          </p>
        </Card>
      )}
    </div>
        ...
        <div className="min-w-0 flex-1">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="mt-0.5 text-sm text-muted">Arrived {formatDate(litter.arrived)}{litter.left_date ? ` · Left ${formatDate(litter.left_date)}` : ''}</p>
            </div>
            <p className="text-right"><span className="text-2xl font-bold text-ink">{litter.kittens.length}</span><span className="ml-1 text-sm text-muted">kittens</span></p>
          </div>
