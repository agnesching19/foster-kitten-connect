  ...
  const content = (
    <>
        ...
        <div className="mt-4 rounded-xl border border-border bg-brand-50 p-4">
          ...
          {summary.skippedLitters ? (
            <p className="mt-1 text-xs text-muted">
              {summary.skippedLitters} existing batch(es) were skipped.
            </p>
          ) : null}
        </div>
      ) : null}
      ...
      <ConfirmDialog
        open={confirmOpen}
        title={
          strategy === 'replace' ? 'Replace existing records?' : 'Merge into existing batches?'
        }
        description={
          strategy === 'replace'
            ? 'Kittens, feedings, bathroom entries, litter box changes, weigh-ins, weights and notes for the matching batches will be deleted and re-imported. This cannot be undone.'
