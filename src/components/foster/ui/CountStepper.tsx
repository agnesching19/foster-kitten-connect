...
export function CountStepper({
  ...
  min?: number
  max?: number
  halfAtMinimum?: boolean
}) {
  const setWithinRange = (nextValue: number) => onChange(Math.max(min, Math.min(max, nextValue)))
  const previousValue = halfAtMinimum && value === 1 ? 0.5 : value - 1
  const nextValue = halfAtMinimum && value === 0.5 ? 1 : value + 1
  ...
  return (
    <div className="grid min-w-0 grid-cols-[3rem_minmax(0,1fr)_3rem] overflow-hidden rounded-xl border border-border bg-white focus-within:border-brand-400 focus-within:ring-2 focus-within:ring-brand-100">
      <button
        type="button"
        onClick={() => setWithinRange(previousValue)}
        disabled={value <= min}
      ...
      <button
        type="button"
        onClick={() => setWithinRange(nextValue)}
        disabled={value >= max}
