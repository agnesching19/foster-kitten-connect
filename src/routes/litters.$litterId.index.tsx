        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {(
            [
              ...
              ['Notes', 'Milestones and observations', 'notes', '📝'],
            ] as const
          ).map(([title, description, section, icon]) => (
            <Link
              key={section}
              to={`/litters/$litterId/${section}` as const}
              params={{ litterId: litter.id }}
              className="rounded-2xl border border-border bg-surface-raised p-4 shadow-sm transition hover:border-brand-300 hover:bg-brand-50"
            >
