-- A weigh-in before a batch's recorded date of birth is invalid comparison
-- data. Older imports may contain these mismatches, so omit them from both the
-- range and its sample-size totals.
...
AS $$
  ...
  observations AS (
    ...
    JOIN public.weigh_ins AS weigh_in
      ...
      AND weigh_in.date >= historical_batch.date_of_birth
